...
for s in sujets:
    for tissue in Tissue_Type:
        ...
        Volume_espace_natif_file_ = pd.read_csv(glob.glob(os.path.join(path_database,s,save_dir_name,'mni_to_native_'+tissue+'*.csv'))[0],delim_whitespace=True)#chargement des donné issue de la derniere etape
        #de RobLob
        ...
        for ind,legend in zip(                       range(len(Volume_espace_natif_file_['Lobe'])),range(len(Legends))                              ):
            ...
            Volume_espace_natif_file_.to_csv(os.path.join(path_database,s,save_dir_name,tissue+'_'+s+'_volume_natif.csv'),index=False)

        
#concatenation des deux tableaux
    volume_grey_matter_lobe = pd.read_csv(os.path.join(path_database,s,save_dir_name,'grey_matter_'+s+'_volume_natif.csv'))
    volume_white_matter_lobe = pd.read_csv(os.path.join(path_database,s,save_dir_name,'white_matter_'+s+'_volume_natif.csv'))
    frames = [volume_grey_matter_lobe, volume_white_matter_lobe]
    Volume_espace_natif_total = pd.concat(frames)
    Volume_espace_natif_total.to_csv(os.path.join(path_database,s,save_dir_name,s+'_volume_lobaire_total_espace_natif.csv'),index=False)
...
for s in sujets:
    volume_lobaire_natif_du_sujet = pd.read_csv(os.path.join(path_database,s,save_dir_name,s+'_volume_lobaire_total_espace_natif.csv'))
    ...
    xlsx_volume_volume_lobaire_natif = volume_lobaire_natif_du_sujet.to_excel(os.path.join(path_database,s,save_dir_name,s+'_volume_lobaire_total_espace_natif.xlsx'),sheet_name='Volume lobaire espace natif',index=False)
